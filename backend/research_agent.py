...
Searches trusted websites via Tavily, extracts content, and uses Gemini 3 Flash
...
CONFIG_PATH = Path(__file__).parent / "config" / "research_config.json"
...
# ---------------------------------------------------------------------------
# OpenRouter client (standalone fallback when llm_client not provided)
# ---------------------------------------------------------------------------

def _get_openrouter_client() -> OpenAI:
    """Create OpenRouter client using environment key."""
    api_key = os.getenv("OPENROUTER_KEY")
    if not api_key:
        raise ValueError("OPENROUTER_KEY not set in environment.")
...
def search_web(
    ...
    Phase 1: Restricted to trusted domains only.
    Phase 2: Fallback broad search with excluded domains filtered out.
...
def build_research_result(
    ...
    return {
        ...
        "pocet_podpornych_zdrojov": llm_response.get("pocet_podpornych_zdrojov", 0),
...
def research_statement(
    ...
    Called when the DB verification pipeline returns no match.
    ...
    # Step 4: LLM analysis
    try:
        llm_response = call_research_llm(llm_client, statement, enriched, config)
