"""Web research agent adapted from Client/research_agent.py.

Uses backend.config for API keys and config paths instead of Client/.env.
"""
...
def search_web(
    ...
    if not TAVILY_API_KEY:
        raise ValueError("TAVILY_API_KEY not configured in backend/.env")
...
def research_statement(
    statement: str, llm_client: OpenAI
) -> dict:
    """Full web research pipeline for a political statement.

    Called when DB search returns no match.
